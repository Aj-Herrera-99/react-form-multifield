...
** dai vari log vedere quali funzioni sono state invocate
** NB: la logica è al 99% centralizzata in App Component
** per semplicità i dati contengono solo 5 proprieta
** per semplicità i dati non contengono la proprieta id (sarebbero da generare dinamicamente, UUID)
...
function App() {
    ...
    // array reattivo degli oggetti formData cui valori sono i campi input finali al submit del form
    const [cards, setCards] = useState([]);
    // array reattivo di valori booleani che si riferiscono (tramite indice) ai tags checkati e no (default: tutti false)
    // ? questi valori reattivi serviranno per controllare le checkbox dei tags
    ...
    const handleInputChange = (e) => {
        ...
        // gli input sono sia text che checkbox, il valore di val dipendera dal tipo => se type = checkbox VAL sara e.target.checked senno sara e.target.value
        ...
        if (name != "tags") return setFormData({ ...formData, [KEY]: VAL }); // (spread) per tutti gli input tranne le checkbox dei tags entra qui
    ...
    // side effects
    //  ? useEffect ha due argomenti: 1) una funzione azione (chiamasi effetto collaterale) 
    // ? 2) un array delle dipendenze
    // ? funzionamento: all'aggiornamento di anche solo uno delle dipendenze, viene invocato l'effetto collaterale
    // * useEffect RUNNA DOPO IL RENDER (NON DURANTE E NON PRIMA)
    useEffect(() => {
        // * controllo preliminare perche tutti gli useEffect partono a prescindere nel primo render
        if (cards.length) {
